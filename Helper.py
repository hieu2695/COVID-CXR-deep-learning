...
#%%------------------ Training processs ---------------------------
def train_model(model, optimizer, criterion, epochs, mode, trainloader, testloader, path):
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    criterion = criterion.to(device)
    count = 0
...
#%% ------------
def evaluation_withloss(model, loader, criterion):
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    criterion = criterion.to(device)
    running_preds = []
    ...
    return sensitivity, specificity, score, test_loss
